...
- **无损云解析**：解析源列表第一项，基于无损云资源库直连播放，自动匹配当前集数；鼠标移到播放器右侧可展开选集面板换集；搜索结果有多个时会弹出选择面板供手动选择（该线路已屏蔽欧美、欧洲线路）
- **手动解析**：点击页面左侧的"VIP"按钮，在解析列表中点击任意解析源即可播放
- **自动解析**：点击"开"按钮启用，页面加载后自动使用选中的解析源播放
- **移动按钮**：右键拖拽VIP按钮到任意位置
...
- **多结果判定**：搜索命中多个结果时，依次尝试集数硬判定、名称信号判定，无法确定时弹出选择面板由你选择
