...
- **多平台支持**：覆盖国内主流视频网站
- **无损云解析**：内置无损云资源库直连，无需第三方解析站，自动匹配当前集数，页面内直接播放（已屏蔽欧美、欧洲线路）
...
- **智能解析**：集成多个解析接口，自动选择最优方案，与无损云解析自由切换互不冲突
- **流程自动取消**：切换解析源时自动中止旧流程（搜索、加载等），避免画面残留和声音叠加
- **自动播放**：支持自动解析和播放功能（默认调用「虾米解析」，也可点击任意其他源固定为自动解析源）
- **响应式设计**：完美适配桌面端和移动端
- **可定制界面**：支持拖拽移动解析按钮位置
- **实时更新**：解析接口定期更新，确保可用性
...
- **无损云解析**：解析源列表第一项，基于无损云资源库直连播放，自动匹配当前集数；鼠标移到播放器右侧可展开选集面板换集；搜索结果有多个时会弹出带详情的面板供手动选择（仅一个精确同名时直接播放）（该线路已屏蔽欧美、欧洲线路）
- **手动解析**：点击页面左侧的"VIP"按钮，在解析列表中点击任意解析源即可播放
- **自动解析**：点击"开"按钮启用，页面加载后自动使用选中的解析源播放（默认「虾米解析」，也可点击任意其他源固定为自动解析源）
- **移动按钮**：右键拖拽VIP按钮到任意位置
...
- **可能原因：**
  - 腾讯视频页面在脚本接管后异步调整播放器容器尺寸，导致播放区域跟着变化
  - 属于站点自身渲染时序问题，概率性触发
- **影响范围：**
  - 仅腾讯视频（`v.qq.com`）桌面端
  - 不影响播放功能，仅右侧区域显示异常（发黑、选集面板偏移、提示偏右）
- **解决方法：**
  - 刷新页面即可恢复
  - 从首页点进视频页通常不会触发此问题
